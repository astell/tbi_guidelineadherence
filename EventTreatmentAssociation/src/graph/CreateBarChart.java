...
public class CreateBarChart {
    ...
    public CreateBarChart(Logger _logger) {
        logger = _logger;
    }

    public void createEventChart(String filepath, TreeMap totalEventNums, String chartTitle) {
        ...
        while (totalEventIter.hasNext()) {
            ...
            //logger.info("totalEventName: " + totalEventName);
        ...
        //logger.info("cppCount: " + cppCount);
        //logger.info("icpCount: " + icpCount);
        ...
        while (totalEventIter.hasNext()) {
            ...
            if (totalEventName.contains("Lowered CPP")) {
                cppValues[cppIndex] = totalEventNum.intValue();
                cppEventNames[cppIndex] = totalEventName;
                cppIndex++;
            } else {
                icpValues[icpIndex] = totalEventNum.intValue();
        ...
        for (int i = 0; i < cppCount; i++) {
            cppDataset.setValue(cppValues[i], "Event number", "" + cppEventNames[i]);
        }
        for (int i = 0; i < icpCount; i++) {
            icpDataset.setValue(icpValues[i], "Event number", "" + icpEventNames[i]);
        }

        JFreeChart cppChart = ChartFactory.createBarChart("" + chartTitle + " (CPP)", "Parameter", "Event number", cppDataset,
                PlotOrientation.VERTICAL, false, true, false);
        int width = 1000;
        int height = 600;
        String filename = filepath + "cpp_bar_chart.png";
        try {
        ...
        JFreeChart icpChart = ChartFactory.createBarChart("" + chartTitle + " (ICP)", "Parameter", "Event number", icpDataset,
                PlotOrientation.VERTICAL, false, true, false);
        width = 1300;
        height = 600;
        filename = filepath + "icp_bar_chart.png";
        try {
    ...
    public void createTimeChart(String filepath, TreeMap timesIn, TreeMap totalEventNums, String chartTitle) {
        ...
        JFreeChart cppChart = ChartFactory.createBarChart("" + chartTitle + " (CPP)", "Parameter", "Time (mins)", cppDataset,
                PlotOrientation.VERTICAL, false, true, false);
        int width = 1000;
        int height = 600;
        String filename = filepath + "cpp_bar_chart.png";
        ...
        JFreeChart icpChart = ChartFactory.createBarChart("" + chartTitle + " (ICP)", "Parameter", "Time (mins)", icpDataset,
                PlotOrientation.VERTICAL, false, true, false);
        width = 1300;
        height = 600;
        filename = filepath + "icp_bar_chart.png";
